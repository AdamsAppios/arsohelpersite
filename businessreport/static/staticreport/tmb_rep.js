...
TmbReportClass.prototype.updateCost = function () {
  var totalCost = 0;
  var expenseAs = "";
  $("table tbody")
    .find("tr")
...
$("#dateReport").change(function () {
  ...
  $.ajax({
    ...
    data: {
      ...
      location: arrLocForms[0].location,
    },
    method: "POST",
    dataType: "json",
    success: function (data) {
      console.log(data);
      globalData = data;
      if (!$.isEmptyObject(globalData)) populateYesterValues();
    },
  });
});
...
$("form input[type=number]").on("blur", function () {
  ...
  new TmbReportClass().updateCTOCalc();
});
